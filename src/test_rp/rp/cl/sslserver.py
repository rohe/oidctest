...
class SecureHTTPServer(ThreadingMixIn, BaseHTTPServer.HTTPServer):
    def __init__(self, server_address, HandlerClass, options):
        ...
        self.socket = ssl.wrap_socket(
            ...
            keyfile=self.options.key, certfile=self.options.cert,
...
class RequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def send_head(self):
        ...
        try:
            ...
            if not ext in ['.html', '.css', '.js', '.json', '.png', '.jpeg',
                           '.jpg']:
                raise IOError
...
# ./sslserver.py -k certs/server.key -c certs/server.crt -p 8088

parser = OptionParser()
parser.add_option('-r', dest='url', default='http://localhost:8332/',
                  help='URL to bitcoin RPC (default: %default)')
parser.add_option('-p', dest='port', type="int", default=8338,
                  help='listen port (default: %default)')
parser.add_option('-n', dest='procs', type="int", default=4,
                  help='number of HTTP processes (default: %default)')
parser.add_option('-k', dest='key', default='server.pem',
                  help='.pem (default: %default)')
parser.add_option('-c', dest='cert', default='server.cert',
                  help='.cert (default: %default)')
